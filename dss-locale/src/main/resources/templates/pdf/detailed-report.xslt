...
<xsl:stylesheet version="1.0"
		...
		<fo:root>
			...
			<fo:page-sequence>
				...
				<fo:flow>
					...
					<xsl:apply-templates
						select="dss:BasicBuildingBlocks[@Type='COUNTER_SIGNATURE']" />
	...
	<xsl:template match="dss:BasicBuildingBlocks">
		<fo:block>
			...
			<xsl:variable name="bbbType">
				<xsl:choose>
					<xsl:when test="@Type = 'TIMESTAMP'">
						<xsl:value-of
							select="../dss:Signatures/dss:ValidationProcessTimestamps[@Id = $bbbId]/@Type" />
					</xsl:when>
					<xsl:otherwise>
			...
			<fo:block>
				...
				<xsl:value-of select="$bbbType" />
				-
				<xsl:value-of select="$bbbId" />
			</fo:block>
		</fo:block>
	...
	<xsl:template match="dss:ValidationProcessBasicSignatures">
		<fo:block>
			<xsl:variable name="indicationText"
				select="dss:Conclusion/dss:Indication/text()" />
	...
	<xsl:template
		match="dss:ISC|dss:VCI|dss:RFC|dss:CV|dss:SAV|dss:XCV|dss:SubXCV|dss:PSV|dss:PCV|dss:VTS">
		<fo:table>
			<xsl:attribute name="page-break-inside">avoid</xsl:attribute>
					...
					<fo:table-cell>
						...
						<fo:block>
							...
							<xsl:attribute name="color"><xsl:value-of
								select="$indicationColor" /></xsl:attribute>
