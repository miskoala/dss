...
<xsl:stylesheet version="1.0"
    ...
    <xsl:template match="dss:Policy">
		<div>
    		...
    		<div>
    			...
    			Polityka weryfikacji : <xsl:value-of select="dss:PolicyName"/>
	        </div>
    		<div>
        ...
        <div>
    		...
    		<div>
	    		...
	    		<xsl:attribute name="data-target">#collapseSig<xsl:value-of select="$idSig" /></xsl:attribute>
		       	<xsl:attribute name="data-toggle">collapse</xsl:attribute>
    			Podpis <xsl:value-of select="$idSig" />
	        </div>
    		<div>
    			<xsl:attribute name="class">panel-body collapse in</xsl:attribute>
		        ...
		        <dl>
		    		<xsl:attribute name="class">dl-horizontal</xsl:attribute>
		            <dt>Czas podpisania:</dt>
		            <dd><xsl:value-of select="dss:SigningTime"/></dd>
		            <dd>[do tłumaczenia] The validation of the signature, of its supporting certificates and of the related certification path has been performed from this reference time.</dd>
		        </dl>
		        
		        <dl>
		    		<xsl:attribute name="class">dl-horizontal</xsl:attribute>
		            <dt>Numer kolejny:</dt>
		            <dd><xsl:value-of select="count(preceding-sibling::dss:Signature) + 1"/> z <xsl:value-of select="count(ancestor::*/dss:Signature)"/></dd>
		        </dl>
		        
		        <xsl:for-each select="dss:SignatureScope">
			        <dl>
			    		<xsl:attribute name="class">dl-horizontal</xsl:attribute>
			            <dt>Zakres podpisu:</dt>
	...
	<xsl:template match="dss:SignatureLevel">
		<dl>
    		<xsl:attribute name="class">dl-horizontal</xsl:attribute>
            <dt>[do tłumaczenia]Qualification:</dt>
            <dd>
				<xsl:value-of select="." />
				<span>
	...
	<xsl:template match="dss:Filename">
		<dl>
    		<xsl:attribute name="class">dl-horizontal</xsl:attribute>
            <dt>[do tłumaczenia]Signature filename:</dt>
            <dd>
					<xsl:value-of select="." />
        	</dd>
        </dl>
	...
	<xsl:template match="dss:SubIndication">
		...
		<dl>
    		<xsl:attribute name="class">dl-horizontal</xsl:attribute>
			<dt>[do tłumaczenia]Sub indication:</dt>
			<dd>
		...
		<div>
    		...
    		<div>
				...
				<xsl:if test="dss:ContainerType">
			        <dl>
			    		<xsl:attribute name="class">dl-horizontal</xsl:attribute>
			            <dt>[Do tłumaczenia]Container type:</dt>
			            <dd><xsl:value-of select="dss:ContainerType"/></dd>
			        </dl>
		        </xsl:if>
	        	<dl>
		    		<xsl:attribute name="class">dl-horizontal</xsl:attribute>
		            <dt>Status podpisów:</dt>
		            <dd>
		                <xsl:choose>
		                ...
		                <xsl:value-of select="dss:ValidSignaturesCount"/> prawidłowych podpisów, z  <xsl:value-of select="dss:SignaturesCount"/>
		            </dd>
		        </dl>
		        <dl>
		    		<xsl:attribute name="class">dl-horizontal</xsl:attribute>
		            <dt>Nazwa dokumentu:</dt>
